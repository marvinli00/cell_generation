...
from models.unet import create_unet_model, load_vae, load_classifier
...
if __name__ == "__main__":
    ...
    model_path = "/scratch/groups/emmalu/marvinli/twisted_diffusion/latent_diffusion_edm/output_crossattn_L/checkpoint-457500/" 
    vae_path = "/scratch/groups/emmalu/marvinli/twisted_diffusion/stable-diffusion-3.5-large-turbo/vae"
    classifier_path = "/scratch/groups/emmalu/marvinli/twisted_diffusion/latent_diffusion_edm/checkpoints_classifier/model_epoch_7.pth"
    ...
    for batch_start in tqdm(range(10, len(df), effective_batch_size), desc="Processing batches"):
        # Create batch from CSV data
        batch = create_batch_from_csv(df, batch_start, effective_batch_size, label_dict, cell_line_dict)

        if batch is None:
            print(f"Skipping batch starting at index {batch_start} due to insufficient data.")
            continue
