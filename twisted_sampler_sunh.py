...
# Define paths to models and checkpoints
model_path = "/scratch/groups/emmalu/marvinli/twisted_diffusion/sunh/two_labels_latent_diffusion_edm_silu_cross_attention/checkpoint-640500"  # Update this to your model checkpoint path
vae_path = "/scratch/groups/emmalu/marvinli/twisted_diffusion/stable-diffusion-3.5-large-turbo/vae"
...
def sample_edm(
    ...
    protein_labels = protein_labels.repeat(batch_size).reshape(-1)
    cell_line_labels = cell_line_labels.repeat(batch_size).reshape(-1)
    ...
    if classifier is not None:
        ...
        reference_channels_for_classifier = torch.cat([reference_channels_for_classifier]*batch_size, dim = 0).to(weight_dtype)
    ...
    for i, t in enumerate(progress_bar):
        ...
        if classifier is not None:
            ...
            # predicted_class = classifier(classifier_input)
...
for batch in tqdm(test_dataloader):
    ...
    wandb.log({"Protein labels": protein_name, "cell labels": cell_line_name})
